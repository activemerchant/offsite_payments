...
class MollieCreditcardNotificationTest < Test::Unit::TestCase
  include OffsitePayments::Integrations

  def setup
    @required_options = { :credential1 => '1234567' }
    @notification = MollieCreditcard::Notification.new("id=tr_h2PhlwFaX8", @required_options)

    MollieCreditcard::API.stubs(:new).with('1234567').returns(@mock_api = mock())
  end
  ...
  def test_raises_without_required_options
    assert_raises(ArgumentError) { MollieCreditcard::Notification.new("", :credential1 => '123') }
    assert_raises(ArgumentError) { MollieCreditcard::Notification.new('id=123', {}) }
  end
