...
class MollieCreditcardHelperTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @helper = MollieCreditcard::Helper.new('order-500','1234567', @required_options)
  ...
  def test_credential_based_url
    @mock_api.expects(:post_request)
      .with('payments', :amount => 500, :description => 'Order #111', :method => 'creditcard', :redirectUrl => 'https://return.com', :metadata => {:order => 'order-500'})
      .returns(CREATE_PAYMENT_RESPONSE_JSON)

    assert_equal Hash.new, @helper.fields
    uri = @helper.credential_based_url

    assert_equal "https://www.mollie.com/paymentscreen/creditcard/testmode/ca8195a3dc8d5cbf2f7b130654abe5a5", uri
  ...
  def test_raises_without_required_options
    assert_raises(ArgumentError) { MollieCreditcard::Helper.new('order-500','1234567', @required_options.merge(:return_url => nil)) }
    assert_raises(ArgumentError) { MollieCreditcard::Helper.new('order-500','1234567', @required_options.merge(:description => nil)) }
  end
  ...
  CREATE_PAYMENT_RESPONSE_JSON = JSON.parse(<<-JSON)
    {
      ...
      "details":null,
      "links":{
        "paymentUrl":"https://www.mollie.com/paymentscreen/creditcard/testmode/ca8195a3dc8d5cbf2f7b130654abe5a5",
